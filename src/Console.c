...
void PrintHelp(void)
{
	if(g_output_message)
	{
		fwprintf(stderr,
			...
			L"  -b x         BAS file character encoding (default: BAS write=UTF8, BAS read=autodetect)\n"
			...
			L"  -1           stop wave (WAV file or WaveIn) processing after loading one file successfully\n"
			L"  -e           exclude BASIC lines (saev only binary data behind the Basic program storage) when\n"
			L"               saving to HEX or BIN file.\n"
			L"  -m x         sets LOMEM (memory start address of BAS program) value for HEX file export\n"
			L"\n"
